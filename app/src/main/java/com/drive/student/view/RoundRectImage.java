...
public class RoundRectImage extends ImageView { // 随便设一个值就好
    ...
    private void initBorderDrawable() {
        ...
        if (borderWidth != 0) {
            ...
            float borderRadiu = circleRadiu == 0 ? 0 : circleRadiu + borderWidth;
            // 设置四角半径集合
            float[] borderRadius = new float[]{borderRadiu, borderRadiu, borderRadiu, borderRadiu, borderRadiu, borderRadiu, borderRadiu, borderRadiu};
            borderDrawable = new ShapeDrawable(new RoundRectShape(borderRadius, null, null));
    ...
    private void initCircleDrawable() {
        float[] circleRadius = new float[]{circleRadiu, circleRadiu, circleRadiu, circleRadiu, circleRadiu, circleRadiu, circleRadiu, circleRadiu};
        ...
        circleDrawable = new ShapeDrawable(new RoundRectShape(circleRadius, null, null));
        ...
        if (getWidth() != 0 && getHeight() != 0) {
            ...
            matrix.setScale(((float) boundWidth) / mBitmap.getWidth(), ((float) boundHeight) / mBitmap.getHeight());
    ...
    private Bitmap getBitmapFromDrawable(Drawable drawable) {
        ...
        try {
            ...
            if (drawable instanceof ColorDrawable) {
                bitmap = Bitmap.createBitmap(DEFAULT_WIDTH, DEFAULT_WIDTH, Bitmap.Config.ARGB_8888);
            } else {
     ...
     * 设置角半径
     *
     * @param circleRadiu 当角半径设置为-1时，并且控件大小为正方形，则图片和边框显示后的效果为圆形
