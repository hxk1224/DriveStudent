...
package com.drive.student.xutils.bitmap.core;
...
import com.drive.student.xutils.bitmap.BitmapGlobalConfig;
import com.drive.student.xutils.bitmap.factory.BitmapFactory;
import com.drive.student.xutils.cache.FileNameGenerator;
import com.drive.student.xutils.cache.LruDiskCache;
import com.drive.student.xutils.cache.LruMemoryCache;
import com.drive.student.xutils.util.IOUtils;
            ...
            try {
                ...
                if (snapshot != null) {
                    ...
                    if (config == null || config.isShowOriginal()) {
                        bitmap = BitmapDecoder.decodeFileDescriptor(snapshot.getInputStream(DISK_CACHE_INDEX).getFD());
                    } else {
                        bitmap = BitmapDecoder.decodeSampledBitmapFromDescriptor(snapshot.getInputStream(DISK_CACHE_INDEX).getFD(), config.getBitmapMaxSize(), config.getBitmapConfig());
                    }
    ...
    private Bitmap decodeBitmapMeta(BitmapMeta bitmapMeta, BitmapDisplayConfig config) throws IOException {
        ...
        if (bitmapMeta.inputStream != null) {
            if (config == null || config.isShowOriginal()) {
                ...
                bitmap = BitmapDecoder.decodeSampledBitmapFromDescriptor(bitmapMeta.inputStream.getFD(), config.getBitmapMaxSize(), config.getBitmapConfig());
            }
        } else if (bitmapMeta.data != null) {
            if (config == null || config.isShowOriginal()) {
                bitmap = BitmapDecoder.decodeByteArray(bitmapMeta.data);
            } else {
                bitmap = BitmapDecoder.decodeSampledBitmapFromByteArray(bitmapMeta.data, config.getBitmapMaxSize(), config.getBitmapConfig());
            }
        }
        return bitmap;
    }
