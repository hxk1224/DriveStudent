...
import com.drive.student.util.cropper.cropwindow.edge.Edge;
...
class VerticalHandleHelper extends HandleHelper {
    ...
    void updateCropWindow(float x, float y, float targetAspectRatio, Rect imageRect, float snapRadius) {
        ...
        final float targetHeight = com.drive.student.util.cropper.util.AspectRatioUtil.calculateHeight(left, right, targetAspectRatio);
        ...
        if (Edge.TOP.isOutsideMargin(imageRect, snapRadius) && !mEdge.isNewRectangleOutOfBounds(Edge.TOP, imageRect, targetAspectRatio)) {
            final float offset = Edge.TOP.snapToRect(imageRect);
            Edge.BOTTOM.offset(-offset);
            mEdge.adjustCoordinate(targetAspectRatio);
        }
        if (Edge.BOTTOM.isOutsideMargin(imageRect, snapRadius) && !mEdge.isNewRectangleOutOfBounds(Edge.BOTTOM, imageRect, targetAspectRatio)) {
