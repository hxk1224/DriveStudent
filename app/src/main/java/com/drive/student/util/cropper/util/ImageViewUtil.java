...
package com.drive.student.util.cropper.util;
...
//		final Rect result = new Rect(resultX, resultY, resultX + (int) Math.ceil(resultWidth), resultY + (int) Math.ceil(resultHeight));
        ...
        double resultWidth;
        double resultHeight;
        double temp1 = (bitmapWidth + 0.0d) / bitmapHeight;
        double temp2 = (viewWidth + 0.0d) / viewHeight;
        if (temp1 >= temp2) {// 图片宽度过高
            resultWidth = viewWidth;
            resultHeight = viewWidth / temp1;
        } else {// 高度过高
            resultHeight = viewHeight;
            resultWidth = viewHeight * temp1;
        }

        int left = (int) Math.round((viewWidth - resultWidth) / 2.0);
        int top = (int) Math.round((viewHeight - resultHeight) / 2.0);
        int right = (int) Math.round(((viewWidth - resultWidth) / 2.0 + resultWidth));
        int bottom = (int) Math.round(((viewHeight - resultHeight) / 2.0 + resultHeight));
        final Rect result = new Rect(left, top, right, bottom);
